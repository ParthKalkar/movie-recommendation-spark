...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val (ratingsPath, doGrading, interactive) = parseArgs(args)
    ...
    val myRating = if (doGrading)
        (new Grader(ratingsPath, sc, interactive)).toRDD
      else
    ...
    val model = ALS.train(train.union(myRating), 8, 10)
  ...
  def parseArgs(args: Array[String]) = {
    ...
    val usageString = "\n\nUsage: path/to/spark-submit path/to/jar movie/lens/data/path -user [true/false] [true/false]\n\n"
